...
Для получения события прибора необходимо более 2-х, в отличие от других запросов.

1. Сформировать запрос на чтение события с помощью функции ceres_q_read_event(...) и отправить его прибору; 

2. Получив ответ, с помощью функции ceres_xxx_event_type(...), где ххх - тип прибора, получить тип события и код события для дальнейшей обработки или значение, говорящее что событий нет;
...
Коды типов событий, сохраняемые функциями ceres_xxx_event_type(...) в переменную event_type_dest:

```cpp
#define CERES_ET_COMMON       0  // общее событие
#define CERES_ET_ALARM        1  // событие тревога
#define CERES_ET_ARM          2  // событие взятие / снятие
#define CERES_ET_TROUBLE      4  // событие неисправность
#define CERES_ET_RECOVERY     8  // событие восстановление
#define CERES_ET_RELAY        16 // событие реле (выхода)
#define CERES_ET_ACCESS       32 // событие СКУД
#define CERES_ET_TECH         64 // событие техлоногическое
```
...
**Внимание! Известная проблема.** Не используйте в С2000-КДЛ адрес 6. Работает не корректно.
